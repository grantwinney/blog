+++
categories = ["Raspberry PI", "Sonic Pi", "Coding"]
date = 2016-06-21T13:02:00Z
description = ""
draft = false
image = "https://images.unsplash.com/photo-1500702790369-fe461685b3c8?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3wxMTc3M3wwfDF8c2VhcmNofDEwfHxtaXhlcnxlbnwwfHx8fDE3MjU5ODMzODh8MA&ixlib=rb-4.0.3&q=80&w=2000"
...
tags = ["Raspberry PI", "Sonic Pi", "Coding"]
title = "Creating Music with Sonic Pi on a Raspberry Pi"

+++


Back in May, at the Stir Trek conference in Columbus OH, I got to watch Scott Fradkin live-code using Sonic Pi for nearly an hour, not only explaining what it was capable of, but showing it too.

He kept building it up as the session went on, and everyone in the room had a chance to see and hear what he was creating. By the end of the session, he had a good beat going! If you want to see the script he used, it's on GitHub.

If you've never heard of it before though, you're probably wondering.. what is Sonic Pi?


What is Sonic Pi?

Sonic Pi is an environment for programming sounds, where one can learn about music and coding at the same time. Sam Aaron started on it to help students in the UK learn computer programming:
...
Sonic Pi was designed with OSC in mind, and is configured to listen for OSC messages on localhost port 4557, so that’s where we send our messages. Nick Johnstone created a ruby gem called sonic-pi-cli, which allows us to send messages via the command line, so let's take advantage of that.

After installing Ruby (not necessary on the Pi), install the gem:
...
Assuming that worked, we still need to find a way to communicate with Sonic Pi from Python. There are packages written for Python that do just that, including python-osc, pyOSC and pyliblo, but let's just build on top of what we’ve already got.
 ...
 * Code Music with Sonic Pi, an e-book by Sam Aaron
 * Overtone, Sonic-Pi’s “big brother”
 * Open Sound Control server and client implementations in pure Python
 * A Ruby CLI for Sonic Pi, created by Nick Johnstone